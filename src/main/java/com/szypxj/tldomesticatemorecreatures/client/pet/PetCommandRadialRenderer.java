...
public final class PetCommandRadialRenderer {
    ...
    private PetCommandRadialRenderer() {
    }

    public static void draw(PoseStack poseStack, int centerX, int centerY, int radius, int configuredDeadZone,
                     int count, int highlightedIndex, boolean showCenterCancel, boolean centerSelected) {
        poseStack.pushPose();
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (selected) {
                drawFilledAnnularSector(
                        poseStack, centerX, centerY, innerRadius, radius,
                        start, end, sectorSteps,
        ...
        if (showCenterCancel) {
            float cancelRadius = cancelRadius(radius, configuredDeadZone);
            if (centerSelected) {
                drawFilledCircle(poseStack, centerX, centerY, cancelRadius,
                        TdmcUiTheme.red(TdmcUiTheme.RADIAL_SELECTED), TdmcUiTheme.green(TdmcUiTheme.RADIAL_SELECTED), TdmcUiTheme.blue(TdmcUiTheme.RADIAL_SELECTED), TdmcUiTheme.RADIAL_ALPHA);
                drawCircleOutline(poseStack, centerX, centerY, cancelRadius,
                        TdmcUiTheme.red(TdmcUiTheme.RADIAL_SELECTED_OUTLINE), TdmcUiTheme.green(TdmcUiTheme.RADIAL_SELECTED_OUTLINE), TdmcUiTheme.blue(TdmcUiTheme.RADIAL_SELECTED_OUTLINE), TdmcUiTheme.RADIAL_ALPHA, 2.0F);
            } else {
                drawFilledCircle(poseStack, centerX, centerY, cancelRadius,
                        TdmcUiTheme.red(TdmcUiTheme.RADIAL_FILL), TdmcUiTheme.green(TdmcUiTheme.RADIAL_FILL), TdmcUiTheme.blue(TdmcUiTheme.RADIAL_FILL), TdmcUiTheme.RADIAL_ALPHA);
                drawCircleOutline(poseStack, centerX, centerY, cancelRadius,
    ...
    public static float cancelRadius(int radius, int configuredDeadZone) {
        float inner = innerRadius(radius, configuredDeadZone);
        return Math.max(16.0F, Math.min(inner - 9.0F, radius * 0.22F));
    }
    ...
    private static void drawFilledCircle(PoseStack poseStack,
